...
export async function fetchEuskotrenL3Data(
    originCode: string,
...
export async function getEuskotrenL3StopTrains(display: Display, maxTrains: number): Promise<MetroStopTrains> {
    ...
    const platform1Trains = await fetchEuskotrenL3Data(display.origin.Code, display.destination.Code, today, maxTrains);
    const platform2Trains = await fetchEuskotrenL3Data(display.destination.Code, display.origin.Code, today, maxTrains);
...
export async function getEuskotrenL3DisplaysTrains(displays: Display[], maxTrains: number = 60): Promise<MetroStopTrains[]> {
    const promises = displays.map(display => getEuskotrenL3StopTrains(display, maxTrains));
